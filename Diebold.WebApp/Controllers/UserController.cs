            ...
            if (ModelState.IsValid)
            {
                ...
                catch (ServiceException serviceException)
                {
                    if (serviceException.InnerException != null)
                    {
                        ...
                        {
                            LogError("Repository Exception occured while creating user", serviceException);
                        ...
                        {
                            LogError("Service Exception occured while creating user", serviceException);
                ...
                {
                    LogError("Exception occured while creating user", e);
                    ModelState.AddModelError("", string.Format("Unexpected LogError: [{0}]", e.Message));
                    ErrorMsg = string.Format("Unexpected LogError: [{0}]", e.Message);
                    return Json(new { success = false, ErrorMsg = ErrorMsg });
                }
            }
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (editItem.OldCompanyId != editItem.CompanyId)
                    {
                        ...
                        foreach (var destinationPI in destinationProperties)
                        {
                            ...
                            if (!(sourcePI.Name.Equals("Links") || sourcePI.Name.Equals("RSSFeeds") || sourcePI.Name.Equals("userPortletsPreferences")))
                            {
                ...
                catch (ServiceException serviceException)
                {
                    if (serviceException.InnerException != null)
                    {
                        ...
                        {
                            LogError("Repository Exception occured while updating user", serviceException);
                            ModelState.AddModelError("ServiceError", string.Format(serviceException.InnerException.Message));
                            ErrorMsg = string.Format(serviceException.InnerException.Message);
                            return Json(new { success = false, ErrorMsg = ErrorMsg });
                        }
                        ...
                        {
                            LogError("Service Exception occured while updating user", serviceException);
                            ModelState.AddModelError("ServiceError", string.Format(serviceException.Message));
                            ErrorMsg = string.Format(serviceException.Message);
                            return Json(new { success = false, ErrorMsg = ErrorMsg });
                        }
            ...
            if (ModelState.IsValid)
            {
                ...
                catch (ServiceException serviceException)
                {
                    if (serviceException.InnerException != null)
                    {
                        ...
                        {
                            LogError("Repository Exception occured while editing user", serviceException);
                            ModelState.AddModelError("ServiceError", string.Format(serviceException.InnerException.Message));
                        }
                        else
                        {
                            LogError("Service Exception occured while editing user", serviceException);
                            ModelState.AddModelError("ServiceError", string.Format(serviceException.Message));
                        }
                    }
                }
                ...
                {
                    LogError("Exception occured while editing user", e);
                    ModelState.AddModelError("", string.Format("Unexpected LogError: [{0}]", e.Message));
                }
            }
            InitializeViewModel(editItem);
            ...
            if (ModelState.IsValid)
            {
                ...
                catch (ServiceException serviceException)
                {
                    if (serviceException.InnerException != null)
                    {
                        ...
                        {
                            LogError("Repository Exception occured while editing user details", serviceException);
                            ModelState.AddModelError("ServiceError", string.Format(serviceException.InnerException.Message));
                        }
                        else
                        {
                            LogError("Service Exception occured while editing user details", serviceException);
                            ModelState.AddModelError("ServiceError", string.Format(serviceException.Message));
                        }
                    }
                }
                ...
                {
                    LogError("Exception occured while editing user details", e);
                    ModelState.AddModelError("", string.Format("Unexpected LogError: [{0}]", e.Message));
                }
            }
            InitializeViewModel(editItem);
